...
8. **mihomo 全局 select 不可与探活并发**——select 改变的是组全局状态，并发下所有在途请求测的是最后选中节点。并发测节点用组测 API `GET /group/{组}/delay`（内核自己测，一次调用全组，天然并发安全；72 节点 5.1s）
9. **探活/HTTP 客户端 catch 块禁止静默吞异常**——本案当初无法定位的直接原因；至少落 logcat，最好把原始错误带回结果 reason 字段
10. **规整测试数据会掩盖并发 bug**——e2e 用均匀/固定数据全绿，并发压力+真实乱序才现形；机械取证 = 沙盒起真内核+真数据+App 精确配置做 A/B 对照
